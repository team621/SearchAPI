...
import static com.gsretail.gst.b2e.search.search.common.WNUtils.*;
...
public class WNSearch {
    ...
    public int setCollectionBasicInfo(int idx) {
        ...
        ret = common.setRanking(wncol.COLLECTIONS[idx], rankingOption[0], rankingOption[1],  parseInt(rankingOption[2], 100));
        ...
        for( String sortField : sortFieldInfo.split(",") )
        {
            ...
            if( checkSortFieldName(wncol.COLLECTIONS[idx], tokens[0], tokens[1]) != 0 )
                return -1;
        }
    ...
    public int setBoostQuery(int idx) {
        ...
        if (!wncol.COLLECTION_INFO[idx][BOOST_QUERY].equals("")) {
				/* if(isDebug) {
					checkCollectionQueryFieldName("[w3SetBoostQuery] [setBoostQuery]", wncol.COLLECTIONS[idx],wncol.COLLECTION_INFO[idx][COLLECTION_QUERY] );
				} */
            ret = common.setBoostQuery(wncol.COLLECTIONS[idx], wncol.COLLECTION_INFO[idx][BOOST_QUERY]);
        }
        return ret;
    }
    ...
    public int setRankRange(int idx) {
        ...
        if (!wncol.COLLECTION_INFO[idx][RANK_RANGE].equals("")) {
            ...
            if(rankFields.length < 2) {
                appendWarnings("[WARNING] [w3SetRankRange] [setRankRange] [" + wncol.COLLECTIONS[idx] + "] RANK_RANGE : '" + wncol.COLLECTION_INFO[idx][RANK_RANGE] + "' error.");
                return -1;
            }
    ...
    public int setFilterQuery(int idx) {
        ...
        if (!wncol.COLLECTION_INFO[idx][FILTER_OPERATION].equals("")) {
            ...
            ret = common.setFilterQuery(wncol.COLLECTIONS[idx], wncol.COLLECTION_INFO[idx][FILTER_OPERATION]);
        }
        return ret;
    }
    ...
    public int setResultFieldInfo(int idx) {
        ...
        int i = checkFieldName("[w3AddDocumentField]", wncol.COLLECTIONS[idx], resultFieldList, RESULT_FIELD);
        if( i != 0 ) {
    ...
    public int getMergeCollIdx(String collName) {
        for(int i=0; i < wncol.MERGE_COLLECTIONS.length; i++) {
            if( wncol.MERGE_COLLECTIONS[i].trim().equals(collName.trim()) ) return i;
        }
        return -1;
    }
    ...
    public String getCollectionName(String col) {
        ...
        for ( int idx=0;idx< wncol.COLLECTIONS.length; idx++){
            if ( wncol.COLLECTIONS[idx].equals(col) ) {
                return wncol.COLLECTIONS_NAME[idx];
            }
        }
